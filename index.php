...
function rj_ot_admin_scriptss(){
  global $pagenow;
    ...
    if ( function_exists( 'wp_enqueue_media' )  &&  did_action( 'wp_enqueue_media' )==false) {
      /* WP 3.5 Media Uploader */
      
      if($_SERVER['SCRIPT_NAME']=='/wp-admin/profile.php' || $_SERVER['SCRIPT_NAME']=='/wp-admin/user-edit.php' || $_SERVER['SCRIPT_NAME']=='/wp-admin/edit-tags.php'  || $_SERVER['SCRIPT_NAME']=='/wp-admin/term.php' ){
...
add_action( 'admin_menu', 'metabox_ui_admin_menus' );

   


function metabox_ui_admin_menus(){
...
$meta_option_pages = (isset($meta_option['pages'])) ? $meta_option['pages'] : '';
...
// disable admin notices for pro purchase
...
function metaboxui_activate() {
        update_option("rj_ot_dismissed_upgrades", '');
}
register_activation_hook( __FILE__, 'metaboxui_activate' );
