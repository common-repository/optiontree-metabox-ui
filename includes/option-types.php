  ...
  function ot_type_rj_location( $args = array() ) {
    ...
    $has_desc = $field_desc ? true : false;
      ...
      if($is_rj_google_map_loded==false){
        ...
        $field_value = ($field_value==$key) ? '' : $field_value;

        $key_url = '&key='.$key;

        $map_api_url = 'https://maps.googleapis.com/maps/api/js?v=3.24&signed_in=true&libraries=places'.$key_url;
        $map_api_url = apply_filters('mu_map_api_url',$map_api_url);
      echo ' <script src="'.$map_api_url.'"></script>';
    ...
    echo '<div class="format-setting type-location ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '" id="' . esc_attr( $field_name ) . '_0_wrap">';
     ...
     echo ' <div id="' . esc_attr( $field_name ) . '_0_locationField">
      <input id="' . esc_attr( $field_name ) . '_0" placeholder="Enter a location"
             ...
             <input id="map_field_' . esc_attr( $field_name ) . '_0" type="hidden" autocomplete="off" name="mui_location_fields[]" value="' . $field_name . '" />
    </div>
...
if ( ! function_exists( 'ot_type_rj_shortcode' ) ) {
  ...
  function ot_type_rj_shortcode( $args = array() ) {
    ...
    $has_desc = $field_desc ? true : false;
      ...
      echo do_shortcode($args['field_std']);
  }
}
