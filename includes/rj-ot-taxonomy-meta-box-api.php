<?php 
if ( ! class_exists( 'RJ_OT_Tax_Meta_Box' ) ) {

  class RJ_OT_Tax_Meta_Box {
    ...
    function __construct( $meta_box ) {
	 ...
	 $taxonomies = isset($this->meta_box['taxonomies']) ?  $this->meta_box['taxonomies'] : array();
        foreach($taxonomies as $taxonomy){
            add_action( "{$taxonomy}_add_form_fields", array( $this, 'add_meta_boxes' ) , 999, 2);
            add_action( "{$taxonomy}_edit_form", array( $this, 'add_meta_boxes' ), 10, 2 );  
            add_action( "edited_{$taxonomy}", array( $this, 'save_meta_box' ) , 10, 2);  
            add_action( "create_{$taxonomy}", array( $this, 'save_meta_box' ), 10, 2 );
     ...
     * @since     1.0
     */
    function build_taxonomy_meta_box( $term, $metabox,$id='',$title='' ) {
		
        $term_id = $term->term_id;
		//$get_cat_meta = get_option("taxonomy_$term_id" );
        if(isset($_REQUEST['tag_ID'])){
		
      echo '
      <style type="text/css">
      .taxonomy_metabox_wrap{
        padding-bottom:20px;
      }
      </style>';
        }
      echo '<div class="taxonomy_metabox_wrap" id="'.$id.'">
	  <h3>'.$title.'</h3>
	  <div class="ot-metabox-wrapper">';
        ...
        foreach ( $this->meta_box['fields'] as $field ) {
          ...
          //$field_value =$get_cat_meta[$field['id']];
    ...
    function save_meta_box( $term_id ) {
		$term_id = ($term_id) ? $term_id : $_REQUEST['tag_ID'];
    ...
    //  if ( $pagenow == 'admin-ajax.php' )
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
         ...
         $field_id = $field['id'];
          ...
          //$old = (isset($cat_meta[$field_id])) ? $cat_meta[$field_id] : ""; 
          $old_term_meta = get_term_meta($term_id,$field_id,true);
          $old = ($old_term_meta !='') ? $old_term_meta : "";  
        $new = '';
        ...
        if ( isset( $_POST[$field['id']] ) ) {
          ...
          $cat_keys = array_keys($_POST);
          if(!empty($cat_keys)){
              foreach($cat_keys as $cat_key){
                  if($cat_key==$field_id){
                     
                      $cat_meta[$cat_key]= $_POST[$cat_key];
                      
                      update_term_meta($term_id,$cat_key,$_POST[$cat_key]);
                  }
      ...
      // removed in wordpress 4.9 and muiot-8.0
 ...
 * @since    2.0
 */
if ( ! function_exists( 'rj_ot_taxonomy_register_meta_box' ) ) {

  function rj_ot_taxonomy_register_meta_box( $args ) {
      if ( ! $args ){
      return;
      }
      
    $ot_meta_box = new RJ_OT_Tax_Meta_Box( $args );
  }
